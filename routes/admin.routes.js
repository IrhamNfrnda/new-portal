...
router.get('/', (req, res) => {
    ...
    db.query('SELECT * FROM news ORDER BY created_at DESC', (err, results) => {
      ...
      res.render('dashboard', { admin: admin, news: results,  successMessage: req.flash('successMessage'), errorMessage : req.flash('successMessage')   });
    });
  });
...
router.post('/add', isAuthenticated, (req, res) => {
  ...
  db.query('INSERT INTO news (title, content, author) VALUES (?, ?, ?)', [title, content, author], (err, results) => {
    if (err) throw err;
...
router.post('/edit/:id', isAuthenticated, (req, res) => {
  const { id } = req.params;
  const { title, content, author } = req.body;
  db.query('UPDATE news SET title = ?, content = ?, author = ? WHERE id = ?', [title, content, author, id], (err, results) => {
    if (err) throw err;
...
router.post('/register', (req, res) => {
    ...
    db.query('SELECT * FROM admin WHERE username = ?', [username], (err, results) => {
      ...
      bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
        ...
        db.query('INSERT INTO admin (username, password) VALUES (?, ?)', [username, hashedPassword], (err, results) => {
          if (err) throw err;
